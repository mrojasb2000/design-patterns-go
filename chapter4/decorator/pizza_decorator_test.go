...
func TestPizzaDecorator_AddIngredient(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When calling the add ingredient of the pizza decorator it must return the text %s the expected test, not '%s'", pizzaResult, expectedText)
	}
}
...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Errorf("When calling AddIngredient on the onion decorator without "+
			"and IngredientAdder in its Ingredient field must return an error, not a string with '%s'", onionResult)
	}
